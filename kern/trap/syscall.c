...
uint32 sys_get_user_hard_limit()
{
	return curenv->user_seglimit;
}

uint32 sys_get_frame_info(uint32 va)
{
	...
	if(ptr_page_table[PTX(va)] & PERM_AVAILABLE)
		return 1;
	return 0;
}
...
void* sys_sbrk(int increment)
{
	...
	if(increment == 0)
		return (void*)env->user_segbrk;
	 if(increment > 0)
	 {
         uint32 newbrk = ROUNDUP(env->user_segbrk + increment, PAGE_SIZE);
		 if(newbrk > env->user_seglimit)
			 return (void*)-1;
		 uint32 oldbrk = env->user_segbrk;
		 env->user_segbrk = newbrk;
		 return (void*)oldbrk;
	 }
     if(increment < 0)
     {
    	 uint32 newbrk = ROUNDDOWN(env->user_segbrk + increment, PAGE_SIZE);
    	 uint32 oldbrk = env->user_segbrk;
    	 env->user_segbrk = env->user_segbrk + increment;
...
uint32 syscall(uint32 syscallno, uint32 a1, uint32 a2, uint32 a3, uint32 a4, uint32 a5)
{
	...
	case SYS_allocate_user_mem:
		...
		return 0;
		break;
	case SYS_get_user_hard_limit:
		return sys_get_user_hard_limit();
	    break;
	case SYS_get_frame_info:
		return sys_get_frame_info(a1);
		break;
