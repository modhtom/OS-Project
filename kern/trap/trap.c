	...
	else
	{
		if (userTrap)
		{
			...
			//your code is here
				...
				int ret = get_page_table(faulted_env->env_page_directory, fault_va, &ptr_page_table);
				uint32 page_table_entry = ptr_page_table[PTX(fault_va)];
				uint32 ispresent = page_table_entry & PERM_PRESENT;
				uint32 isuser = page_table_entry & PERM_USER;
				uint32 ismarked = page_table_entry & PERM_AVAILABLE;
				uint32 iswritable = page_table_entry & PERM_WRITEABLE;
				if((fault_va >= (faulted_env->user_seglimit + PAGE_SIZE)) && (fault_va < USER_HEAP_MAX))
				{
					if(isuser == 0)
					{
				...
				if(isuser == 0 && ispresent == 1)
				{
					sched_kill_env(faulted_env->env_id);
				}
				if(iswritable == 0 && ispresent == 1)
				{
					sched_kill_env(faulted_env->env_id);
				}
			/*============================================================================================*/
