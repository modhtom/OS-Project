...
inline struct WorkingSetElement* env_page_ws_list_create_element(struct Env* e, uint32 virtual_address)
{
	...
	if(!(virtual_address >= 0 && virtual_address < USER_TOP))
	{
		panic("WS element creation failure");
	}
	struct WorkingSetElement* ws_element = (struct WorkingSetElement*)kmalloc(sizeof(struct WorkingSetElement));
	ws_element->empty = 0;
	ws_element->virtual_address = ROUNDDOWN(virtual_address,PAGE_SIZE);
	ws_element->prev_next_info.le_next = NULL;
	ws_element->prev_next_info.le_prev = NULL;
	ws_elements_arr[ws_element->virtual_address >> 12] = (uint32)ws_element;
	return ws_element;
}
