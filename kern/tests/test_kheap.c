...
int test_kheap_virt_addr()
{
			...
			for (j = PTX(va); i < numOfFrames && j < 1024 && va < endVA; ++j, ++i)
			{
				...
				if (retrievedVA != (va+offset))
				{
					if (correct)
					{
						...
						correct = 0; cprintf("Wrong kheap_virtual_address 1\n");
					}
				}
				va+=PAGE_SIZE;
			}
	...
	//test kheap_virtual_address after kmalloc and kfree [20%]
	{
		...
		for (i = startIndex ; i < startIndex + 4*Mega/PAGE_SIZE; ++i)
		{
			...
			{
				if (correct)
				{ correct = 0; cprintf("Wrong kheap_virtual_address 2\n"); }
			}
		...
		for (i = startIndex + 4*Mega/PAGE_SIZE; i < startIndex + (7*Mega + 8*kilo)/PAGE_SIZE; ++i)
		{
			...
			{
				if (correct)
				{ correct = 0; cprintf("Wrong kheap_virtual_address 3\n"); }
			}
		}
		//frames of 6 MB
		for (i = startIndex + (7*Mega + 8*kilo)/PAGE_SIZE; i < startIndex + (13*Mega + 8*kilo)/PAGE_SIZE; ++i)
		{
			...
			{
				if (correct)
				{ correct = 0; cprintf("Wrong kheap_virtual_address 4\n"); }
			}
		}
		...
		for (i = startIndex + (13*Mega + 8*kilo)/PAGE_SIZE; i < startIndex + (13*Mega + 24*kilo)/PAGE_SIZE; ++i)
		{
			...
			{
				if (correct)
				{ correct = 0; cprintf("Wrong kheap_virtual_address 5\n"); }
			}
		}
	}
	if (correct)	eval+=20 ;
		...
		for (va = KERNEL_HEAP_START; va < (uint32)sbrk(0); va++)
		{
			...
			if (retrievedVA != va)
			{
				if (correct)
				{
					...
					correct = 0; cprintf("Wrong kheap_virtual_address 6\n");
				}
			}
		}
	}
	...
	//test kheap_virtual_address on frames of KERNEL CODE [20%]
	{
		...
		for (i = 1*Mega; i < (uint32)(end_of_kernel - KERNEL_BASE); i+=PAGE_SIZE)
		{
			...
			if (retrievedVA != 0)
			{
				...
				{
					cprintf("\nPA = %x, retrievedVA = %x\n", i, retrievedVA);
					correct = 0; cprintf("Wrong kheap_virtual_address 7\n");
				}
			}
		}
	}
