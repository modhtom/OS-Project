...
void test_realloc_block_FF()
{
	...
	*(endVAs[idx]) = idx ;
	...
	//====================================================================//
	//[1.1] Test realloc by passing VA & size equals to the old size. It should do nothing and return same given VA
	//====================================================================//
	cprintf("      new => 1.1: Test calling realloc with VA & SIZE = size of block itself.\n\n") ;
	va = realloc_block_FF(startVAs[idx], actualSize);
	if(va == NULL || (va != (curVA + sizeOfMetaData()))) // check return Address
		panic("test_realloc_block_FF %d : WRONG : return Address!", 1.1);
	if(*(startVAs[idx]) != idx || *(midVAs[idx]) != idx || *(endVAs[idx]) != idx) // check content
	{
		panic("test_realloc_block_FF %d : WRONG : content!", 1.1);

	}
	if(get_block_size(va) != get_block_size(startVAs[idx]) || is_free_block(va) != 0) // check size & status
	{
		panic("test_realloc_block_FF %d : WRONG : size & status!", 1.1);
	...
	//[3.1] reallocate in same place (NO relocate - split - No coalesce)
	cprintf("	3.1: reallocate in same place (NO relocate - split - No coalesce)\n\n") ;
	is_correct = 1;
	{
		blockIndex = 4*allocCntPerSize - 1 ;
	...
	//[3.1.1] reallocate in same place (NO relocate - split - coalesce)
	cprintf("	    new => 3.1.1: reallocate in same place (NO relocate - No split - coalesce)\n\n") ;

	//[3.1.2] reallocate in same place (NO relocate - No split - No coalesce - Internal Fragmentation)
	cprintf("	    new => 3.1.2: reallocate in same place (NO relocate - No split - No coalesce - Internal Fragmentation)\n\n") ;
	...
	//[3.3] reallocate in other place (relocate)
	cprintf("	new => 3.3: reallocate in other place (relocate)\n\n") ;

	//[4] Test realloc with decreased sizes
	cprintf("4: Test calling realloc with decreased sizes.[30%]\n\n") ;
	//[4.1] next block is full (NO coalesce - split)
	...
	//[4.1.1] next block is Full (NO coalesce - NO split - Internal fragmentation)
	cprintf("	    new => 4.1.1: next block is Full (NO coalesce - NO split - Internal fragmentation)\n\n") ;

	//[4.2] next block is Null (NO coalesce - split)
	cprintf("	new => 4.2: next block is Null (NO coalesce - split)\n\n") ;

	//[4.2.1] next block is Null (NO coalesce- NO split - Internal fragmentation)
	cprintf("	    new => 4.2.1: next block is Null (NO coalesce - NO split - Internal fragmentation)\n\n") ;

	//[4.3] next block is free (coalesce)
	cprintf("	new => 4.3: next block is free (coalesce)\n\n") ;
