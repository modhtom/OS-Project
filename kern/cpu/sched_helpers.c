...
void env_set_nice(struct Env* e, int nice_value)
{
	...
	if(nice_value > 20)
		nice_value = 20;
	e->nice = nice_value;
	if(find_env_in_queue(&env_new_queue, e->env_id) == NULL)
	{
		...
		fixed_point_t sec_part = fix_int(nice_value);
		e->recent_cpu = fix_add(fst_part, sec_part).f;
		e->priority = fix_trunc(fix_sub(fix_int((PRI_MAX - (nice_value * 2))), fix_unscale(__mk_fix(e->recent_cpu), 4)));
...
int env_get_recent_cpu(struct Env* e)
{
	...
	return fix_round(fix_scale(__mk_fix(e->recent_cpu), 100));
}
int get_load_average()
{
	//TODO: [PROJECT'23.MS3 - #3] [2] BSD SCHEDULER - get_load_average
	return fix_round(fix_scale(__mk_fix(load_average), 100));
}
/********* for BSD Priority Scheduler *************/
//==================================================================================//
